...
interface Row { id: number; date: string; narration: string; accountName: string; debit: number; credit: number; runningBalance: number }
...
export default function BankStatement() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const bankIds = new Set(accts.docs.filter((a) => a.class === 'bank').map((a) => a.id))
      ...
      const result: Row[] = []; let running = 0
      ...
      setRows(result)
    } catch (err: unknown) { setError(err instanceof Error ? err.message : 'Failed to load bank statement') } finally { setLoading(false) }
  }, [from, to, tenantId])
  ...
  const csv = () => downloadCsv('bank-statement.csv', ['Date', 'Narration', 'Account', 'Debit', 'Credit', 'Running'],
      ...
      {loading ? <ReportSkeleton sections={1} /> : (
        <>
          <div className="mt-4 grid grid-cols-3 gap-4">
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-4">
              <div className="text-xs uppercase tracking-wide text-slate-500">Total Withdrawals</div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-left text-xs uppercase tracking-wide text-slate-500">
                  <th className="px-4 py-2">Date</th><th className="px-4 py-2">Narration</th><th className="px-4 py-2">Account</th>
                  <th className="px-4 py-2 text-right">Debit</th><th className="px-4 py-2 text-right">Credit</th><th className="px-4 py-2 text-right">Running</th>
                </tr>
              </thead>
              <tbody>
                {rows.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-400">No bank transactions found.</td></tr>}
                {rows.map((r, i) => (
