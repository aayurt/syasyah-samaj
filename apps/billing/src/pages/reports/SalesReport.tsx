...
export default function SalesReport() {
  ...
  const kpis = [
    { label: 'Total Entries', value: String(invoices.length), sub: `${creditNotes.length} return${creditNotes.length !== 1 ? 's' : ''}`, href: drillTo('sales-invoice') },
    { label: 'Total Sales', value: fmt(totalSales), href: drillTo('sales-invoice') },
    { label: 'Received', value: fmt(totalReceived), href: `/vouchers?type=receipt-voucher${from ? `&from=${from}` : ''}${to ? `&to=${to}` : ''}` },
    ...
    <div className="mx-auto max-w-6xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold text-slate-900">{t('reports.salesReport', 'Sales Report')}</h1>
        </div>
