...
export default function TaxPurchase() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      for (const doc of d.docs) {
        if (doc.docType !== 'purchase-invoice' && doc.docType !== 'payment-voucher') continue
      ...
      setError(err instanceof Error ? err.message : 'Failed to load tax purchases')
    ...
    <div className="mx-auto max-w-6xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold text-slate-900">{t('reports.taxPurchase', 'Tax Purchase')}</h1>
        </div>
      ...
      {loading && entries.length === 0 ? <ReportSkeleton sections={1} /> : (
        <>
          <div className="mt-4 grid grid-cols-2 gap-4 lg:grid-cols-3">
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-4">
              <div className="text-xs uppercase tracking-wide text-slate-500">Total Tax (TDS/VAT)</div>
              <div className="mt-1 font-mono text-lg font-semibold text-amber-700">{fmt(totals.tax)}</div>
            </div>
          </div>
