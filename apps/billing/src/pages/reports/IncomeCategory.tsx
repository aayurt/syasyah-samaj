...
import type { Account, AccountGroup, PnlResponse } from '../../lib/types'
...
export default function IncomeCategory() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      for (const row of pnl.income) {
        ...
        byGroup.get(groupName)!.accounts.push({ name: acc?.code ? `${acc.code} · ${row.account.name}` : row.account.name, amount })
      }
      const cats = Array.from(byGroup.values()).map((g) => ({ ...g, total: g.accounts.reduce((s, a) => s + a.amount, 0) })).sort((a, b) => b.total - a.total)
      setCategories(cats); setTotal(grandTotal)
      setExpanded(new Set(cats.slice(0, 3).map((c) => c.name)))
    } catch (err: unknown) { setError(err instanceof Error ? err.message : 'Failed to load income categories') } finally { setLoading(false) }
  }, [from, to, tenantId])
  ...
  const csv = () => downloadCsv('income-category.csv', ['Category', 'Account', 'Amount'],
    categories.flatMap((c) => c.accounts.map((a) => [c.name, a.name, a.amount])))
  const pdf = () => exportReportPdf({
    filename: 'income-category.pdf', title: 'Income Categories',
    meta: [['From', from || 'Earliest'], ['To', to || 'Latest'], ['Total Income', fmt(total)]],
    tables: categories.map((c) => ({ title: `${c.name} (${fmt(c.total)})`, columns: ['Account', 'Amount'], rows: c.accounts.map((a) => [a.name, a.amount]), totals: ['Subtotal', c.total] })),
    foot: [{ label: 'Total Income', value: total }],
  })
    ...
    <div className="mx-auto max-w-5xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold text-slate-900">{t('reports.incomeCategory', 'Income Category') }</h1>
        </div>
      ...
      {loading && categories.length === 0 ? <ReportSkeleton sections={2} /> : (
        <>
          <div className="mt-4 rounded-lg border border-slate-200 bg-white p-4">
            ...
            <div className="mt-1 font-mono text-xl font-semibold text-emerald-700">{fmt(total)}</div>
          </div>
          <div className="mt-4 space-y-2">
            ...
            {categories.length === 0 && <p className="text-center text-sm text-slate-400">No income categories found.</p>}
          </div>
        </>
      )}
    </div>
